...
export function handleWebhookEvent(app: App): void {
  ...
  // Adjust the error handler to handle the error without needing a specific type from Webhooks
  app.webhooks.onError((error: any) => {
    // `error` here can be any Error object, so we log it in a generic way
    const errorMessage = `Error processing request: ${error.message || error}`;
    app.octokit.log.error(errorMessage, error);
  });
}
